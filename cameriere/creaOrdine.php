	...
	if($_SESSION['ruolo']=="C")
    {
        ...
        {                   
          ...
          { 
            ...
            $note=$_POST['note'];
            $tot=$prezzo*$quantita;
            $query="INSERT INTO `ordini` (`pietanza`, `tavolo`, `quantita`, `note`, `cameriere`, `data`, `ora`, `tot`, `stato`) VALUES ('".$pietanza."', '".$_GET['idTav']."', '".$quantita."', '".$note."', '".$_SESSION['idStaff']."', '".date('d/m/Y')."', '".date('H:i')."', '".$tot."', '0');";
            $db->query($query);
            $str=NULL;
            $pietanza=NULL;
            $prezzo=NULL;
...
<?php 
                              $queryO="SELECT * FROM ordini WHERE stato=0 AND tavolo=".$_GET['idTav']." AND data='".date('d/m/Y')."'";
...
?>
                                        ...
                                        <tr class="bg-light text-dark text-center"><td colspan="6"><a href="procediAllOrdine.php?idTav=<?php echo $_GET['idTav']; ?>"><button class="btn btn-success">Procedi all'ordine</button></a></td></tr>
<?php                                    
                                  }                 
                                  else
                                  {
