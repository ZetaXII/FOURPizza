	...
	if($_SESSION['ruolo']=="C")
    {
    	...
    	if(isset($_POST['modifica']))
        {
          ...
          if($controllaEmail->num_rows>=1)
          {
			 ...
			 echo "&nbsp;<a href='infoCameriere.php'><button class='btn btn-danger' style='margin:10px auto;'>Indietro</button></a>
          ...
          else if($_FILES['img']['tmp_name']!=NULL && $imgCanc!='on')
          {
            ...
            if($grandezzaImg<=64)
            {
              ...
              $query="UPDATE staff set nome='".$nome."' , cognome='".$cognome."' , email='".$email."' , nick='".$nick."' , password='".$password."' , cellulare=".$cellulare." , immagine='".$immagine."' WHERE idStaff=".$_SESSION['idStaff'];
              $db->query($query);
              $_SESSION['immagine']=$immagine;
          	  header("location: homeCameriere.php");                              
            }
            else
            {
              include("navCameriere.php");
          ...
          else
          {
            ...
            {
          		$query="UPDATE staff set nome='".$nome."' , cognome='".$cognome."' , email='".$email."' , nick='".$nick."' , password='".$password."' , cellulare=".$cellulare." WHERE idStaff=".$_SESSION['idStaff'];
                $db->query($query);          
          		header("location: homeCameriere.php");
            }
          }
                	...
                	<table class="text-left" style="margin: 0px auto !important;">
                    	<tr>
                        	<th colspan="2" class="text-center">
                                ...
                                {
                                	echo "<img src='data:image/jpeg;base64,".$_SESSION['immagine']."' style='width:64px; height:64px; border-radius:80px; border:1px solid gray'/>";
                                }
                                else
                                {
                                	echo "<img src='../img/standardImg.jpg' style='width:64px; height:64px; border-radius:80px;'/>";
                                } 
                                ?>
                        	</th>
                        </tr>
                    	...
                    	<tr><th>Ruolo</th>        
                        <td><input type="radio" class="radio-inline" value="A" disabled>&nbsp;Admin&nbsp;<input type="radio" value="P" disabled>&nbsp;Pizzaiolo&nbsp;<input type="radio" value="C" checked="checked" disabled>&nbsp;Cameriere</td></tr>				
